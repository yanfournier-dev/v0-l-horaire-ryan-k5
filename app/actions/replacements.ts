...
export async function getUserApplications(userId: number) {
  try {
    ...
    console.error("[v0] getUserApplications: Error", error)
    return []
  }
}
...
export async function getRecentReplacements() {
  try {
    ...
    console.error("[v0] getRecentReplacements: Error", error)
    return []
  }
}
...
export async function getAllReplacements() {
  try {
    ...
    console.error("[v0] getAllReplacements: Error", error)
    return []
  }
}
...
export async function approveApplication(applicationId: number, replacementId: number) {
  ...
  try {
    const appResult = await sql`
      SELECT ra.applicant_id, r.shift_date, r.shift_type, r.team_id
      FROM replacement_applications ra
      JOIN replacements r ON ra.replacement_id = r.id
    ...
    const { applicant_id: applicantId, shift_date, shift_type, team_id } = appResult[0]
